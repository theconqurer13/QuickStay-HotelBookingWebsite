...
const HotelReg = () => {
    ...
    const onSubmitHandler = async (e) => {
        try {
            e.preventDefault();
            const {data} = await axios.post('/api/hotels/',{name,contact,address,city},{headers:{Authorization:`Bearer ${getToken()}`}});
        ...
        <form onSubmit={onSubmitHandler} onClick={(e)=> e.stopPropagation()} className='flex bg-white rounded-xl max-w-4xl max:md:mx-2'>
            ...
            <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
                ...
                <div className='w-full mt-4 max-w-60 mr-auto'>
                    <label htmlFor="city" onChange={(e)=> setCity(e.target.value)} value={city} className='font-medium text-gray-500 ' >City</label>
