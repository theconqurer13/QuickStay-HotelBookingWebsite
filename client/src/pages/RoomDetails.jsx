...
const RoomDetails = () => {
    ...
    const checkAvailability = async()=> {
        try {
            // checkIn date is greator than checkOut date
    ...
    // onSubmitHandler to check availability and book room
  ...
  return room && (
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        ...
        <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-8 lg:gap-12
        bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
            ...
            <div className='flex flex-col flex-wrap md:flex-wrap items-start md:items-center gap-4 md:gap-10 text-gray-500'>
                <div className='flex flex-col '>
                    ...
                    <input onChange={(e)=>setGuests(e.target.value)} value={guests} type="number" max={4} min={1} id='guests' placeholder='1'  className='w-full rounded border border-gray-300 px-3 py-2 mt-1.5 outline-none' required/>
                </div>
            </div>
            <button type='submit' className='bg-black hover:bg-primary-dull active:scale-95 transition-all text-white rounded-md max-md:w-full max-md:mt-6 md:px-25 py-3 md:py-4 text-base cursor-pointer md:ml-auto lg:ml-auto hover:opacity-80 duration-200'>
                {isAvailable ? 'Book Now' : 'Check Avilability'}
            </button>
        </form>
